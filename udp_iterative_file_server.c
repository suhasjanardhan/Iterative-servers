...
/*
 main - Connectionless Iterative servers
 */
...
int accept_socket,sock,data_len,alen;
...
sock=connectUDP(service,portnum);
printf("Listening to client\n");
		...
		while(1)
		{
		        ...
		        alen=sizeof(fsin);
			data_len = recvfrom(sock,msg,MAX_DATA,0,(struct sockaddr *)&fsin,&alen);        //to recieve the connection from client
			
			
			if(data_len)
			{
				printf("connected to iterative connectionless server\n");
